...
API_KEY = "bb46b8017fb2bf54d8343c00f2435ef4"
...
def wav_get(word):
    r = requests.post(URL, headers=getHeader(), data=getBody(word))
    ...
    #   合成音频格式为pcm、wav并保存在audio目录下
            writeFile("./wav/" + word + ".mp3", r.content)
        else:
            # print(r.content)
    #   合成音频格式为mp3并保存在audio目录下
            writeFile("./wav/" + "xiaoyan" + ".wav", r.content)
        # print("success, sid = " + word)
    else:
    #   错误码链接：https://www.xfyun.cn/document/error-code （code返回错误码时必看）
        print(r.text)
