...
def process(input_file, output_file):
    # 打开待处理文本，划分单词
    with open(input_file, encoding='UTF-8') as raw_text:
        words = re.split('[^a-zA-Z]', raw_text.read().lower())
    ...
    while not unknown.empty():
        ...
        if choice:
            ...
            f.write('**' + word.name + '**: ' + word.eng_interpretation + '\n')
            print("生词")
        else:
            old_words.append(word)
            print("熟词")
        print("========剩余{}个单词=========".format(unknown.qsize()))
